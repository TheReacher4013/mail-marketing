...
const Automation = {
    findAll: (limit, offset) =>
        pool.query(
            `SELECT a.id, a.name, a.trigger_event, a.is_active, a.created_at, u.name AS created_by_name FROM automations a JOIN users u ON a.created_by=u.id ORDER BY a.created_at DESC LIMIT ? OFFSET ?`,
            [limit, offset]
        ),
        ...
        findById: (id) => 
            ...
            create: (data, userId) => pool.query(
                ...
                [data.name || null, data.trigger_event || null, data.workflow_json ? JSON.stringify(data.workflow_json): null,
                    data.is_active ?? null, id
                ]
            ),

            delete:(id) => pool.query(`DELETE FROM automation WHERE id=?`, [id]),
